...
do
    ...
    MM2AdminPanel.Name = "CATHUB"
    MM2AdminPanel.ResetOnSpawn = false
    MM2AdminPanel.IgnoreGuiInset = false
    MM2AdminPanel.DisplayOrder = 999

    -- Scale the whole hub to fit phones/tablets while preserving the original layout.
    local uiScale = MM2AdminPanel:FindFirstChild("MobileUIScale")
    if not uiScale then
    ...
    local function updateMobileScale()
        ...
        for _, obj in ipairs(MM2AdminPanel:GetDescendants()) do
    ...
    local logo = MM2AdminPanel:FindFirstChild("CATHUB_Logo")
    if not logo then
        logo = Instance.new("ImageButton")
        logo.Name = "CATHUB_Logo"
        logo.Parent = MM2AdminPanel
