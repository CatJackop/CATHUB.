...
--========================================================--
-- CAT HUB : Floating Icon Shape Changer
-- Changes the hide/show floating button shape.
-- Existing icon/image and button behavior remain intact.
--========================================================--

_G.CatHubIconShape = _G.CatHubIconShape or "Circle"

local function ApplyCatHubIconShape(shape)
    _G.CatHubIconShape = shape

    -- Look for the existing floating icon/button created by CAT HUB.
    -- This only changes its corner radius; it does not replace its callbacks.
    pcall(function()
        ...
        for _, gui in ipairs(pg:GetDescendants()) do
            if gui:IsA("ImageButton") or gui:IsA("TextButton") then
                ...
                if string.find(n, "icon", 1, true)
                    or string.find(n, "toggle", 1, true)
                    ...
                    elseif shape == "Curved Square" then
...
Tabs.Settings:AddDropdown({
    Name = "Floating Icon Shape",
    Description = "Choose the CAT HUB hide/show icon shape",
    Options = {"Circle", "Curved Square", "Square"},
    Default = "Circle",
    Callback = function(Value)
        ApplyCatHubIconShape(Value)
    end
})

task.defer(function()
    task.wait(1)
    ApplyCatHubIconShape(_G.CatHubIconShape)
end)
...
--========================================================--
-- CAT HUB : Fishing Notifications
--========================================================--

_G.FishingNotifications = true

Tabs.Fishing:AddToggle({
    Name = "Fishing Notifications",
    Description = "Show notifications for fishing actions",
    Default = true,
    Callback = function(Value)
        _G.FishingNotifications = Value
    end
})

local function FishingNotify(title, message, duration)
    if not _G.FishingNotifications then return end
    pcall(function()
        Window:Notify({
            Title = title,
            Content = message,
...
FishingNotify("🎣 Fishing Ready", "Fishing notifications are enabled.", 4)
...
BuyBaitToggle = Tabs.Fish:AddToggle({
    ...
    Callback = function(Value)
        ...
        FishingNotify("🪱 Auto Buy Bait", Value and "Enabled" or "Disabled", 3)
        if Value then
            pcall(function()
                Remotes.RFCraft:InvokeServer("Craft", _G.SelectedBait, {})
            end)
...
FishingToggle = Tabs.Fish:AddToggle({
    Name = "Auto Fishing",
    Description = "Automatically cast and catch fish",
    Default = false,
    Callback = function(Value)
        _G.AutoFishing = Value
        FishingNotify("🎣 Auto Fishing", Value and "Enabled" or "Disabled", 3)
    end
})

-- Improved fishing controller: equips the selected rod, finds a safe water
-- position in front of the player, and handles casting/catching states.
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
...
local MaxDistance = Config.Rod.MaxLaunchDistance or 100

local function getCharacter()
    return LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
end

local function getFishingTool(Char)
    local selected = _G.SelectedRod or "Fishing Rod"
    local tool = Char:FindFirstChildOfClass("Tool")

    if tool and tool.Name == selected then
        return tool
    end

    local backpackTool = LocalPlayer.Backpack:FindFirstChild(selected)
    if backpackTool and Char:FindFirstChildOfClass("Humanoid") then
        Char.Humanoid:EquipTool(backpackTool)
        task.wait(0.15)
        return Char:FindFirstChildOfClass("Tool")
    end

    -- Fallback: use any fishing rod if the selected rod is unavailable.
    if not tool then
...
local function getWaterTarget(Char, HRP)
    local origin = Char:FindFirstChild("Head") and Char.Head.Position or HRP.Position
    local direction = HRP.CFrame.LookVector * MaxDistance
    local params = RaycastParams.new()
    params.FilterType = Enum.RaycastFilterType.Exclude
    params.FilterDescendantsInstances = {Char, Workspace.Characters, Workspace.Enemies}
    params.IgnoreWater = false

    local result = Workspace:Raycast(origin, direction, params)
    local hitPos = result and result.Position

    -- If the ray does not hit water, use a point in front of the player and
    -- place it at the calculated water height. This avoids getting stuck.
    local forwardPos = HRP.Position + HRP.CFrame.LookVector * math.min(MaxDistance * 0.75, 80)
    local samplePos = hitPos or forwardPos

    local ok, waterHeight = pcall(function()
        return GetWaterHeight(samplePos)
    end)
    if not ok or typeof(waterHeight) ~= "number" then
        return nil
    end

    return Vector3.new(samplePos.X, math.max(samplePos.Y, waterHeight), samplePos.Z)
end
    ...
    while task.wait(0.35) do
        if _G.AutoFishing then
            pcall(function()
                ...
                local Tool = getFishingTool(Char)
                ...
                if State == "ReeledIn" or ServerState == "ReeledIn" then
                    local TargetPos = getWaterTarget(Char, HRP)
                    if TargetPos then
...
local GachaTravel = false

Tabs.Raids:AddToggle({
Name = "Instant/Tween to Gacha",
Description = "Go to the Random Fruit gacha NPC before buying",
Default = false,
Callback = function(Value)
  GachaTravel = Value
end})

local function GoToRandomFruitGacha()
  local npc
  pcall(function() npc = replicated.NPCs:FindFirstChild("Barista Cousin") end)
  if not npc then pcall(function() npc = workspace:FindFirstChild("Barista Cousin", true) end) end
  if npc and npc:FindFirstChild("HumanoidRootPart") and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
...
spawn(function()
  while wait(Sec) do
      ...
      if _G.Random_Auto then
        if GachaTravel then GoToRandomFruitGacha() end
        replicated.Remotes.CommF_:InvokeServer("Cousin","Buy")
      end 
    end)
  end
...
--========================================================--
-- CAT HUB : Fruit Spawn Notifier
-- Notifies you when a physical fruit appears in this server.
-- This section is independent of the existing farming/attack code.
--========================================================--

_G.FruitSpawnNotifier = true

Tabs.Settings:AddToggle({
    Name = "Fruit Spawn Notifier",
    Description = "Notify when a fruit is spawned in this server",
    Default = true,
    Callback = function(Value)
        _G.FruitSpawnNotifier = Value
    end
})

local SeenSpawnedFruits = {}

local function IsFruitObject(obj)
    if not obj then return false end

    local name = tostring(obj.Name or "")
    if string.find(string.lower(name), "fruit", 1, true) then
        return true
    end

    -- Some fruit objects keep their display/original name in an attribute.
    local originalName = obj:GetAttribute("OriginalName")
    if originalName and string.find(string.lower(tostring(originalName)), "fruit", 1, true) then
        return true
    end
...
local function NotifyFruitSpawn(obj)
    if not _G.FruitSpawnNotifier or not obj or SeenSpawnedFruits[obj] then
        return
    end
    ...
    SeenSpawnedFruits[obj] = true

    pcall(function()
        Window:Notify({
            Title = "🍎 FRUIT SPAWNED!",
            Content = tostring(obj.Name) .. " has spawned in this server!",
            Image = "rbxassetid://72243650390596",
            Duration = 7
        })
    end)
    ...
    task.delay(30, function()
        SeenSpawnedFruits[obj] = nil
    end)
end

workspace.DescendantAdded:Connect(function(obj)
    task.defer(function()
        NotifyFruitSpawn(obj)
    end)
end)
...
--========================================================--
-- CAT HUB : Selected-NPC Attack Kill (OWN GAME)
-- Requires ReplicatedStorage.AttackKillRemote (RemoteEvent)
--========================================================--

local ReplicatedStorage = game:GetService("ReplicatedStorage")
local AttackKillRemote = ReplicatedStorage:WaitForChild("AttackKillRemote")

_G.AttackKillEnabled = _G.AttackKillEnabled or false
_G.AttackKillRadius = _G.AttackKillRadius or 25
_G.AttackKillNPC = _G.AttackKillNPC or "All NPCs"

local function AttackKillNotify(message)
    pcall(function()
        Window:Notify({
            Title = "⚔️ Attack Kill",
            Content = message,
...
Tabs.Main:AddSection("Attack Kill")

Tabs.Main:AddToggle({
    Name = "Attack Kill",
    Description = "Enable/disable selected-NPC attack kill",
    Default = _G.AttackKillEnabled,
    Callback = function(Value)
        _G.AttackKillEnabled = Value
        AttackKillRemote:FireServer("Toggle", Value)
        AttackKillNotify(Value and "Enabled" or "Disabled")
    end
})

Tabs.Main:AddSlider({
    Name = "Kill Radius",
    Description = "Attack range for selected NPCs",
    Min = 5,
    Max = 100,
    Default = _G.AttackKillRadius,
    Rounding = 0,
    Callback = function(Value)
        _G.AttackKillRadius = Value
        AttackKillRemote:FireServer("Radius", Value)
    end
})

-- Populate the selector from NPC models currently in Workspace.
local function GetNPCNames()
    ...
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj:IsA("Model")
            ...
            and not game:GetService("Players"):GetPlayerFromCharacter(obj) then
    ...
    table.sort(names)
    table.insert(names, 1, "All NPCs")

    return names
end

local NPCOptions = GetNPCNames()

Tabs.Main:AddDropdown({
    Name = "NPC Selection",
    Description = "Choose which NPC type Attack Kill targets",
    Options = NPCOptions,
    Default = _G.AttackKillNPC,
    Callback = function(Value)
        _G.AttackKillNPC = Value
        AttackKillRemote:FireServer("NPC", Value)
        AttackKillNotify("Target: " .. tostring(Value))
    end
})

-- Sends only a request; the server performs the actual NPC validation and kill.
task.spawn(function()
    while task.wait(0.15) do
        if _G.AttackKillEnabled then
            AttackKillRemote:FireServer("Kill")
        end
    end
end)
...
--========================================================--
-- SERVER SCRIPT
-- Place this portion in ServerScriptService in YOUR OWN GAME.
--========================================================--
...
local remote = ReplicatedStorage:WaitForChild("AttackKillRemote")
...
remote.OnServerEvent:Connect(function(player, action, value)
    ...
    elseif action == "Radius" then
        s.radius = math.clamp(tonumber(value) or 25, 5, 100)

    elseif action == "NPC" then
        s.npc = tostring(value)

    elseif action == "Kill" then
        if not s.enabled then return end

        local now = os.clock()
        if now - s.lastKill < 0.12 then return end
        s.lastKill = now
        ...
        for _, npc in ipairs(workspace:GetChildren()) do
            ...
            if validNPC(npc)
                ...
                and (s.npc == "All NPCs" or npc.Name == s.npc)
                and (root.Position - npcRoot.Position).Magnitude <= s.radius then
