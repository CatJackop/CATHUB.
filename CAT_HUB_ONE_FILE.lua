...
local CATHUB = Instance.new("ScreenGui")
...
CATHUB.Name = "CAT HUB"
CATHUB.Parent = game.CoreGui
CATHUB.ResetOnSpawn = false

MainFrame.Name = "MainFrame"
MainFrame.Parent = CATHUB
...
-- CAT HUB Mobile UI + Hide/Show Logo
do
    local UserInputService = game:GetService("UserInputService")
    ...
    CATHUB.Name = "CAT HUB"
    CATHUB.ResetOnSpawn = false
    CATHUB.IgnoreGuiInset = false
    CATHUB.DisplayOrder = 999

    -- Scale the whole hub to fit phones/tablets while preserving the original layout.
    local uiScale = CATHUB:FindFirstChild("MobileUIScale")
    if not uiScale then
    ...
    local function updateMobileScale()
        ...
        local targetWidth = 430
        local targetHeight = 300

        -- Keep a comfortable margin on small screens.
        local scaleX = (viewport.X - 24) / targetWidth
        local scaleY = (viewport.Y - 70) / targetHeight
        ...
        local minButton = touch and 28 or 20

        for _, obj in ipairs(CATHUB:GetDescendants()) do
            if obj:IsA("TextButton") or obj:IsA("ImageButton") then
                local current = obj.Size
                if current.X.Offset > 0 and current.Y.Offset > 0 then
                    obj:SetAttribute("OriginalTouchSizeX", obj:GetAttribute("OriginalTouchSizeX") or current.X.Offset)
                    obj:SetAttribute("OriginalTouchSizeY", obj:GetAttribute("OriginalTouchSizeY") or current.Y.Offset)
                    local ox = obj:GetAttribute("OriginalTouchSizeX")
                    local oy = obj:GetAttribute("OriginalTouchSizeY")
                    if touch and (ox < minButton or oy < minButton) then
                        obj.Size = UDim2.new(current.X.Scale, math.max(ox, minButton), current.Y.Scale, math.max(oy, minButton))
                    end
                end
    ...
    -- Dedicated draggable logo for opening/closing CAT HUB.
    -- Kept inside this single CAT HUB ScreenGui, but outside MainFrame,
    -- so hiding MainFrame does not hide the logo.
    local logo = CATHUB:FindFirstChild("CATHUB_Logo")
    if logo then logo:Destroy() end

    logo = Instance.new("ImageButton")
    logo.Name = "CATHUB_Logo"
    logo.Parent = CATHUB
    ...
    logo.Image = "rbxassetid://126031329785796"
    ...
    -- Make the top bar easier to drag on mobile.
    TopFrame.Active = true
    TopFrame.Selectable = true

    -- Avoid the original 500x500 intro overflowing on small screens.
    task.delay(0.05, function()
        if StartFrame then
            StartFrame.Size = UDim2.fromOffset(220, 220)
        end
        updateMobileScale()
    end)
end
