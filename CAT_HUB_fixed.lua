...
-- ========================================
-- CAT HUB PERFORMANCE / ANTI-SPAM HOOKS
...
-- ========================================
-- CAT HUB FULL BRIGHT - brighter, readable lighting.
-- ========================================
pcall(function()
...
-- ========================================
-- CAT HUB AUTO KEN (Observation Haki).
...
if not (getrawmetatable and setreadonly and newcclosure and getnamecallmethod) then
  warn("[CAT HUB] executor lacks metatable hook API; aim/mouse-position hook disabled")
else
pcall(function()
...
local Fluent
do
  -- Multiple direct sources: raw files are tried first because some executors
  -- cannot follow GitHub release redirects.
  local sources = {
  ...
  -- One short retry helps mobile executors that initialise HTTP a moment late.
  if not Fluent then
    task.wait(1)
    for _, url in ipairs(sources) do
      local ok, body = pcall(httpget, url)
  ...
  if not Fluent then
    local msg = "[CAT HUB] UI library could not load: " .. tostring(lastErr)
    warn(msg)
    pcall(function()
      game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "CAT HUB",
        Text = "UI could not start because the UI library was blocked by this executor.",
        Duration = 12
      })
    end)
    return
...
--local LocalPlayer = game:GetService("Players").LocalPlayer -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.

-- unique-flag generator (Fluent wants a unique flag id per stateful element)
local _flagN = 0
local function nextFlag(prefix)
    _flagN = _flagN + 1
    return "CAT HUB_" .. (prefix or "F") .. "_" .. _flagN
end
...
-- CAT HUB floating open/close GUI button.
...
function WindowWrap:BuildFloatingButton()
    ...
    backgroundImage.Image = "rbxassetid://126031329785796"
...
-- the custom CAT HUB floating button built inside MakeWindow.
...
-- ---- CAT HUB GUI entry point --------------------------------------------
local NevaLib = {}
function NevaLib:MakeWindow(o)
    -- CAT HUB custom themes
    pcall(function()
    ...
    local w = Fluent:CreateWindow({
        Title = "CAT HUB",
        SubTitle = "IS BACK | created by catjack.gg",
...
-- ================= CAT HUB window bootstrap =================
if not Fluent then
    warn("[CAT HUB] Startup stopped safely: Fluent UI is unavailable.")
    return
end
...
-- CAT HUB config manager sections (interface / save / floating button / media).
-- Everything is kept inside ONE parent folder ("CAT HUB") so the executor's
...
-- so loading now would skip them. See CAT HUB_DeferredAutoload() at the file end.
CAT HUB_DeferredAutoload = function() end -- replaced below if managers exist
pcall(function()
    ...
    if MediaManager then
        MediaManager:SetFolder("CAT HUB/Media")
    end
    ...
    -- the single "CAT HUB" folder remains. Only deletes them if they are empty
    -- / safe, and only if the executor supports folder deletion.
    pcall(function()
        if not (isfolder and delfolder) then return end
        local strays = { "FluentSettings", "FloatingButtons", "ImageCache", "FluentShowcase", "CAT HUBv3" }
    ...
    CAT HUB_DeferredAutoload = function()
...
Tabs.Info:AddDiscordInvite({
	...
	Description = "A community for CAT HUB Users - official scripts, updates, and support in one place. Created by catjack.gg.",
	Banner = "rbxassetid://75841821715476", 
	Logo = "rbxassetid://75841821715476",
	Invite = "https://discord.gg/yuaWVkXFQH",
	Members = 26000, 
	Online = 2800, 
})
...
--local Players = game:GetService("Players") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
--local LocalPlayer = Players.LocalPlayer -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
local Workspace = game:GetService("Workspace")
--local ReplicatedStorage = game:GetService("ReplicatedStorage") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
...
--local plr = game.Players.LocalPlayer -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
...
--local Players = game:GetService("Players") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
--local UserInputService = game:GetService("UserInputService") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
--local RunService = game:GetService("RunService") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
...
--local replicated = game:GetService("ReplicatedStorage") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
...
--local player = game.Players.LocalPlayer -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
...
--local Players = GameService:GetService("Players") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
--local RunService = GameService:GetService("RunService") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
--local ReplicatedStorage = GameService:GetService("ReplicatedStorage") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
--local Workspace = GameService:GetService("Workspace") -- [CAT HUB FIX] duplicate top-level local removed: the chunk was at 199/200 registers (Luau's hard limit is 200). Already declared identically earlier.
...
pcall(function()
    if type(CAT HUB_DeferredAutoload) == "function" then
        CAT HUB_DeferredAutoload()
    end
end)

-- ========================================
-- CAT HUB AUTO-SAVE (true persistence for ALL toggles/sliders/dropdowns)
-- Auto-manages a config named "CAT HUB Config":
...
pcall(function()
    ...
    local AUTO_NAME = "CAT HUB Config"
    local folder = SaveManager.Folder or "CAT HUB"
...
Window:Notify({
  Title = "CAT HUB",
  Content = "CAT HUB is back - created by catjack.gg",
  Image = "rbxassetid://75841821715476",
  Duration = 5
})