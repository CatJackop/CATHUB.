...
-- ============================================================================
-- CAT HUB - NPC RADIUS CONTROL
-- Added as a standalone feature. Existing CAT HUB code is left unchanged.
-- Toggle: turns the NPC radius check ON/OFF.
-- Slider: changes the radius in studs while the toggle is enabled.
-- ============================================================================

local NPC_Radius = 25
local NPC_Radius_Enabled = false

Tabs.Main:AddSection("NPC Radius Control")

Tabs.Main:AddToggle({
    Name = "Instant Kill NPCs In Radius",
    Description = "Toggle automatic NPC health reduction inside the selected radius.",
    Default = false,
    Callback = function(Value)
        NPC_Radius_Enabled = Value
    end
})

Tabs.Main:AddSlider({
    Name = "NPC Kill Radius",
    Description = "Adjust the radius in studs.",
    Min = 1,
    Max = 500,
    Default = 25,
    Increment = 1,
...
local function KillNPCsInRadius()
    if not NPC_Radius_Enabled then return end
    ...
    for _, model in ipairs(workspace:GetChildren()) do
        if model:IsA("Model") and model ~= character then
            -- Ignore player characters; only process NPC models.
            if not ply:GetPlayerFromCharacter(model) then
...
task.spawn(function()
    while task.wait(0.1) do
        pcall(KillNPCsInRadius)
    end
end)

-- ============================================================================
-- End of standalone NPC Radius Control
-- ============================================================================
